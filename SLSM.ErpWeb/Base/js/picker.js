...
layui.define(['laytpl', 'form'], function (exports) {
    ...
    Picker.fn.render = function () {
        ...
        var config = that.config,
            ...
            getAreaCodeByPathName = function (options) {
                if (DATA.length === 0)
                    throw new Error('PICKER ERROR:请设置数据源.');
                var data = DATA;
                var result = undefined;
                var ArrayPathChina = options.pathChina.split(',');
                if (ArrayPathChina.length != 4) {
                    return undefined;
                }
                for (var i = 0; i < data.length; i++) {
                    var e = data[i];
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (e.type != 2)
                        continue;
                    if (e.name == ArrayPathChina[1] && e.code.substring(0, 2) == result.code.substring(0, 2)) {
                        result = e;
                        break;
                    }
                }
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (e.type != 3)
                        continue;
                    if (e.name == ArrayPathChina[2] && e.code.substring(0, 4) == result.code.substring(0, 4)) {
                        result = e;
                        break;
                    }
                }
            ...
            temp = function (filterName, tipInfo) {
                ...
                html += '<option value="{{ item.code }}_{{ item.code }}" selected>{{ item.name }}</option>';
                html += '{{# }else{ }}'
                html += '<option value="{{ item.code }}_{{ item.code }}">{{ item.name }}</option>';
                html += '{{# }; }}';
                html += '{{#  }); }}';
                html += '</select>';
                html += '</div>';
                    ...
                    form.on('select(' + filter + ')', function (data) {
                        ...
                        switch (data.elem.name) {
                            case pickerFilter.province:
                                renderData(pickerType.city, $picker, data.value.split("_")[1], undefined, false);
                                break;
                            case pickerFilter.city:
                                renderData(pickerType.area, $picker, data.value.split("_")[1], undefined, false);
                                break;
        ...
        if (config.codeConfig) {
            ...
            if (path == undefined) {
                path = getAreaCodeByPathName(config.codeConfig);
            }
