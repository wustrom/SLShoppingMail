...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        var waitLoadIndex;
        if (data.Closeabled == true) {
            tabIndex = -1;
            _config.elem.children('ul.layui-tab-title').children('li').children('cite').each(function () {
                if (data.title == $(this).html().trim()) {
                    $(this).parent().find('.layui-tab-close').trigger('click');
                }
            });
        }
        if (tabIndex === -1) {
