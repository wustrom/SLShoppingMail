layui.config({
    ...
    var btable = layui.btable(),
        ...
        form = layui.form();
    var configOption = new Object();
    var BoxIndex = -1;
    btable.set({
        openWait: true,//开启等待框
        elem: '#content',
        url: '../../../APi/Table/GetStorageInfo?time=' + (new Date().getTime()), //数据源地址
        pageSize: 18,//页大小
        params: {//额外的请求参数
            t: new Date().getTime()
        },
        columns: [
            ...
            },
            {
                ...
                field: 'Ids',
                format: function (val, obj) {
                    var html = '<button type="button" value="调整库存" data-action="edit" data-id="' + val + '" data-matercolorId="' + obj.matercolorId + '" class="layui-btn layui-btn-small"><img src="../Current/Icon/StockbackGreen.png" />&nbsp; 调整库存</button>';
                    return html;
                },
                width: "10%"
            }],
                    ...
                    $that.on('click', function () {
                        ...
                        switch (action) {
                            ...
                            case 'edit':
                                ...
                                var matercolorId = $that[0].dataset.matercolorid;
                                $.get('../Warehouses/StorageEditInfo?Id=' + Id + '&matercolorId=' + matercolorId, null, function (form) {
    ...
    form.on('submit(search)', function (data) {
        configOption = data.field;
        btable.get(configOption);
        return false;
    });
