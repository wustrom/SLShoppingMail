layui.config({
    ...
    btable.set({
        openWait: true,//开启等待框
        elem: '#content',
        url: '../../../APi/Table/GetDesigner', //数据源地址
        pageSize: 18,//页大小
        params: {//额外的请求参数
            t: new Date().getTime()
        },
        columns: [
            ...
            },
            {
                ...
                format: function (val, obj) {
                    if (obj.DesignerStatus == "设计未处理") {
                        var html = '<label style="color:#7cc0ff;">' + obj.DesignerStatus + '</label>';
                    }
                    else if (obj.DesignerStatus == "生产退回待处理") {
                        var html = '<label style="color:#ff663a;">' + obj.DesignerStatus + '</label>';
                    }
                    else {
                        var html = '<label style="color:black;">' + obj.DesignerStatus + '</label>';
                    }
                    return html;
                },
                width: "13%"
                ...
                $(this).children('td').children('button').each(function () {
                    ...
                    $that.on('click', function () {
                        ...
                        switch (action) {
                            ...
                            case 'Detailed':
                                parent.tab.tabAdd({
                                    href: '../Designer/ProductionInfo?Id=' + Id,
                                    title: '订单详情',
                                    Closeabled: true
                                });
                            //#endregion                
