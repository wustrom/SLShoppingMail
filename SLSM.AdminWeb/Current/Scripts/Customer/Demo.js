...
//#region添加客服人员到数据库
$(function () {
    $('.webim-button:last').click(function () {
        var ServiceName = $('.webim-button:last').prevAll('input:last').val();
        var ServicePwd = $('.webim-button:last').prevAll('input:first').val();
        var datas = { ServiceName: ServiceName, ServicePwd: ServicePwd, IsService: false };
        $.ajax({
            type: "post",
            data: JSON.stringify(datas),
            url: "../../api/Service/AddServiceInfo",
            dataType: "json",
...
//#region登录时状态改为工作中
$(function () {
    $('.webim-button').click(function () {
        var name = $('.webim-input:first').val();
        $('.Status').html("工作中");
        $('.Status').css("background-color", "green");
        datas = { ServiceName: name, IsService: false };
        $.ajax({
            type: "post",
            data: JSON.stringify(datas),
            url: "../../api/Service/EditServiceInfo",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                if (data.HttpCode != 200) {
...
$(function () {
    $('.Status').click(function () {
        ...
        if (status == "工作中") {
            ...
            datas = { ServiceName: name, IsService: true };
