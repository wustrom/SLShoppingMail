/// <reference path="../../current/scripts/ajax.js" />
...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (columns[i].sortable) {
                th += '<th data-name="' + columns[i].field + '"' + width + '>' + columns[i].fieldName + '<div class="btable-order"><div class="up" title="升序"><i class="fa fa-sort-asc" aria-hidden="true"></i></div ><div class="down" title="倒序"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div></th>';
            }
            else if (columns[i].showable == false) {
                th += '<th ' + width + ' style="display: none;" >' + columns[i].fieldName + '</th>';
            }
            else {
                th += '<th ' + width + '>' + columns[i].fieldName + '</th>';
            }
        }
        if (_config.SerialNumber) {
        ...
        paging.init({
                ...
                for (var i = 0; i < columns.length; i++) {
                    if (columns[i].format) {
                        ...
                        $('#' + dataId).find('tr').each(function () {
                            var id = $(this).find('input[data-item=id]').data('id');
            ...
            success: function () { //完成的回调
                ...
                //重新渲染复选框
                $(_config.elem).find('tbody > tr').each(function () {
                    var $that = $(this);
                    $(this).find("input[type='button']").each(function () {
                        var thisinfo = $(this);
                        var dataaction = $(this).attr('data-action');
                        if (dataaction == _config.dbclicktext && _config.dbclicktext != undefined) {
                            $that.on('dblclick', function () {$(thisinfo).trigger("click");});
                        }
                    })
                    $(this).find("button[type='button']").each(function () {
                        var thisinfo = $(this);
                        var dataaction = $(this).attr('data-action');
                        if (dataaction == _config.dbclicktext && _config.dbclicktext != undefined) {
                            $that.on('dblclick', function () {$(thisinfo).trigger("click");});
                ...
                form.on('checkbox(singleselector)', function (data) {
                    var elem = data.elem;
                    $(elem)[0].checked = (elem.checked == true ? false : true);
    ...
    BTable.prototype.reload = function (options) {
        paging.reload(options);
    }
	/**
	 * 获取选择的行。
	 */
    ...
    function getTpl(options) {
        ...
        var tds = '';
        for (var i = 0; i < columns.length; i++) {
            if (columns[i].showable == false) {
                tds += '<td style="display: none;" data-field="' + columns[i].field + '">{{ item.' + columns[i].field + ' }}</td>';
            }
            else {
        ...
        if (options.SerialNumber) {
            tds = '<td>{{ (' + options.pageSize + '*(d.pageno-1)+(index+1)) }}</td>' + tds
        }
        if (options.checkbox) {
            tds = '<td><input type="checkbox" data-item="id" data-id="{{ item.' + options.field + ' }}" lay-filter="singleselector" lay-skin="primary" /></td>' + tds;
        } else {
    ...
    BTable.prototype.ajaxPost = function (url, data, successfn) {
